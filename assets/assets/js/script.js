...
async function loadMyBills() {
    ...
    if (tbody) {
        tbody.innerHTML = '<tr><td colspan="6" class="loading">Loading bills...</td></tr>';
    }
    ...
    try {
        ...
        const allBills = JSON.parse(localStorage.getItem('bills') || '[]');
        let myBills = allBills.filter(bill => {
            const billTimestamp = new Date(bill.timestamp).getTime();
            return billTimestamp >= todayTimestamp && bill.cashierId === state.currentUser.id;
        });
...
async function viewBillDetails(billNumber) {
    ...
    try {
        ...
        // Try to find bill in localStorage first
        const localBills = JSON.parse(localStorage.getItem('bills') || '[]');
        billData = localBills.find(bill => bill.billNumber === billNumber);

        // If not found locally and Firebase is available, try Firebase
        if (!billData && isFirebaseInitialized) {
            try {
